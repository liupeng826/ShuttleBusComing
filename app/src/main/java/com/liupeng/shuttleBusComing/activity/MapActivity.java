package com.liupeng.shuttleBusComing.activity;
...
import static com.liupeng.shuttleBusComing.R.id.map;
...
public class MapActivity extends Activity implements LocationSource,
    ...
    private void init() {
        ...
        mSelectedBusLineNumber = settings.getInt(mLineKey, 0);
        ...
        mDriver.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mDriver.isChecked()) {
                    new AlertDialog.Builder(MapActivity.this).setTitle("老司机")//设置对话框标题
        ...
        call.enqueue(new Callback<CoordinateGson>() {
            ...
            public void onResponse(Call<CoordinateGson> call, Response<CoordinateGson> response) {
                ...
                if(response.body().getData().size() > 1){
                    Toast.makeText(getApplicationContext(), mSelectedBusLine.replace("Bus", "") + "号线 有多个司机，请关闭多余司机", Toast.LENGTH_SHORT).show();
                    return;
                }
...
//                });
    ...
    public void postLocation(LatLng myLocation) {
        ...
        if(mDriverChecked){
            coordinate.setRole(mSelectedBusLine);
        }
    ...
    public void updateDriver() {
        ...
        Coordinate coordinate = new Coordinate();
        coordinate.setUser(getuuid(this));
        coordinate.setRole(String.valueOf(mSelectedBusLineNumber));

        Call<Coordinate> call = apiManager.updateDriver(coordinate);
        call.enqueue(new Callback<Coordinate>() {
            @Override
            public void onResponse(Call<Coordinate> call, Response<Coordinate> response) {
                //处理请求成功
